...
- **팀원 모델 차등은 아직 열지 않는다.** 비용 실측에서 팀의 출력 토큰이 단일의 10배 이상이고 그 대부분이 팀원 쪽에서 나오므로, 값싼 모델로 팀원을 돌리는 것은 **효과가 클 가능성이 높다.** 다만 `SubAgentSpec`에 `model` 필드를 더하는 스키마 변경(SPEC_VERSION 0.2 → 0.3)이라 수정이 아니라 기능이다 — 다음 Phase 항목으로 둔다
