...
- **평가 케이스 5건은 적고, 전부 통과한다 — 현재 회귀 감지력이 없다.** CLI·UI 양쪽에서 5/5(2026-08-10). 모두 통과하는 스위트는 **나빠질 때만** 알려주므로, "Builder가 잘한다"는 보여주되 "회귀를 잡는다"는 증명되지 않았다. 다음 작업은 케이스 추가이고, **추측으로 정답을 적지 말고** Builder를 실제로 틀리게 만드는 요구를 먼저 찾아야 한다:
  - 모호한 요구 (도구 선택이 갈리는 것)
  - 팀/단일 판단이 애매한 중간 난이도
  - 사용자가 명시적으로 금지한 것을 지키는지 (`calculator_no_search`가 유일한 예)
  - 실패 케이스를 발견하면 그것을 케이스로 고정하는 것이 정석 순서다
- ~~**심판 신뢰도 미검증**~~ → 2026-08-10 해소. 부실 스펙 1/5, 무관 스펙 1/5로 판별력 확인 (위 실호출 검증 참조). 다만 Builder와 심판이 **같은 모델**(둘 다 기본 `claude-sonnet-4-6`)인 상태라 자기 채점 편향은 완전히 배제되지 않았다 — `DEEP_BUILDER_JUDGE_MODEL`로 다른 모델을 지정해 교차 확인하면 더 확실하다
