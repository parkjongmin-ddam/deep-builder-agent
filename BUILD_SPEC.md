...
- **평가 케이스를 계속 늘려야 한다.** 12건이 다시 전부 통과하므로, 지금 이 스위트는 이미 고친 결함이 되돌아오는 것만 잡는다. 절차는 확립됐다 — **기대값을 먼저 적고, 후보 요구를 실제로 던지고, 어긋난 것만 케이스로 고정한다.** 2차 확장에서 쓴 축(모순 요구·화이트리스트 밖 능력·MCP 얽힘·도구 3개 이상)은 소진했다. 다음 축 후보: 사용자가 도구를 직접 지정하는 요구, 팀원 간 의존이 있는 요구(A의 결과가 B의 입력), 출력 형식이 까다로운 요구(표·JSON), 다국어 요구
